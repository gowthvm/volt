      ...
      <div className="mx-auto max-w-6xl px-5">
        ...
        <div className="grid gap-px overflow-hidden rounded-lg border border-default bg-border-subtle sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="feature-card animate-fade-slide-up group relative bg-base p-8 transition-all duration-300 hover:-translate-y-1 hover:shadow-accent"
              style={{ opacity: 0 }}
            >
              <div className="mb-4 flex h-9 w-9 items-center justify-center rounded-lg border border-default bg-surface text-text-tertiary transition-all duration-300 group-hover:-translate-y-0.5 group-hover:border-accent/30 group-hover:text-accent">
                {f.icon}
              </div>
              ...
              <h3 className="relative mb-1.5 text-[15px] font-semibold text-text-primary">{f.title}</h3>
              <p className="relative text-[13px] leading-relaxed text-text-tertiary">{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
